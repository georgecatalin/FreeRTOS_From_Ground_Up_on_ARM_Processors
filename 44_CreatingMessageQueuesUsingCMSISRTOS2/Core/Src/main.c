...
osMessageQueueId_t my_queue = NULL;

uint16_t message_to_send[1] = {1978};
uint16_t message_to_receive[1];
...
int main(void)
{
  ...
  //Create QUEUE
  const osMessageQueueAttr_t queue_attributes = { .name="attributes"};
  my_queue = osMessageQueueNew(5, sizeof(uint16_t), &queue_attributes);
...
void orangeTask(void *pvParams)
{
	...
	while(1)
	{
		...
		osMessageQueuePut(my_queue, message_to_send, 0, osWaitForever);
...
void whiteTask(void *pvParams)
{
	...
	while(1)
	{
		...
		osMessageQueueGet(my_queue, message_to_receive, 0, osWaitForever);
		data_received = message_to_receive[0];
