...
int main(void)
{
	...
	xTaskCreate(vBlueLedControllerTask, "BlueLed Task", 100, NULL, 1, &blue_handle);
	xTaskCreate(vRedLedControllerTask, "RedLed Task", 100, NULL, 1, &red_handle);
	xTaskCreate(vGreenLedControllerTask, "GreenLed Task", 100, NULL, 1, &green_handle);
...
void vRedLedControllerTask(void *pvParameters)
{
	while(1)
	{
		...
		if(execution_monitor>=60)
		{
			is_killed = true;
			execution_monitor=0;
			vTaskDelete(NULL); // OR vTaskSuspend(red_handle);
