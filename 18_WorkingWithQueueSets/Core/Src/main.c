...
int main(void)
{
    ...
    xQueueSet = xQueueCreateSet(2*1); // contains 2 Queues of 1 element each
...
void vSenderTask2(void *pvParams)
{
    ...
    char * msg = "Welcome from Sender2 Task\r\n";
...
void vReceiverTask(void *pvParams)
{
    ...
    while(1)
    {
    	xQueueThatContainsData =(QueueHandle_t ) xQueueSelectFromSet(xQueueSet, portMAX_DELAY); // This value represents the longest possible delay
    	xQueueReceive(xQueueThatContainsData, &recvdmsg, 0);
    	printf("%s" , recvdmsg);
    }
}
