...
typedef struct
{
	uint8_t ucSensorValue;
	sDataSourceFrom xSensorType;
} Data_t;

static const Data_t xArrayToWorkWith[2] = { {12, humidity_sensor} , {45, pressure_sensor} }; //this will contain data to be passed through queue
...
int main(void)
{
	...
	//Create the Queue
	xQueue = xQueueCreate(3, sizeof(Data_t)); //Queue will hold 3 elements of type Data_t


	//Create the tasks

	xTaskCreate(SenderTask, "Humidity Sender Task", 100, (void *) &(xArrayToWorkWith[0]), 2, &humidity_task_handle);
	xTaskCreate(SenderTask, "Pressure Sender Task", 100, (void *) &(xArrayToWorkWith[1]), 2, &pressure_task_handle);
	xTaskCreate(ReceiverTask, "Receiver Task", 100, NULL, 1, &receiver_task_handle);
...
//Detail the Sender and Receiver Tasks
void SenderTask(void *pvParams)
{
	BaseType_t qStatus;
...
void ReceiverTask(void *pvParams)
{
	...
	while(1)
	{
		...
		if(qStatus == pdPASS)
		{
			if(xStructReceived.xSensorType == humidity_sensor)
			{
				printf("The value read by the humidity sensor = %d. \r\n", xStructReceived.ucSensorValue);
			}
			else
			{
