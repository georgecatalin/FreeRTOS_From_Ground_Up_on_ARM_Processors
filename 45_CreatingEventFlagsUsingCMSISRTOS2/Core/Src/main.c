...
osEventFlagsId_t eventFlags_1 = NULL;
...
int main(void)
{
  ...
  //Create EventFlag objec
  const osEventFlagsAttr_t event_attributes = { .name="attributes"};
  eventFlags_1 = osEventFlagsNew(&event_attributes);
...
void orangeTask(void *pvParams)
{
	...
	while(1)
	{
        ...
        osEventFlagsSet(eventFlags_1, 0x01);
...
void whiteTask(void *pvParams)
{
	...
	while(1)
	{
        ...
        osEventFlagsWait(eventFlags_1, 0x01, osFlagsWaitAny, osWaitForever);
        printf("Running this in the White Task after receiving the flag 0x01 \n\r");
