...
osSemaphoreId_t semaphore_1 = NULL;
...
int main(void)
{
  ...
  //Create semaphore
  const osSemaphoreAttr_t  semaphore_attributes = { .name="attributes"};
  semaphore_1 = osSemaphoreNew(1, 1, &semaphore_attributes);
...
void orangeTask(void *pvParams)
{
	while(1)
	{
		...
		osSemaphoreAcquire(semaphore_1, osWaitForever);
