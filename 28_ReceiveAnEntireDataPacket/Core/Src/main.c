...
#define EXPECTED_PACKET_SIZE 5

int rxInProgress = 0;
uint16_t rxLength = 0;
uint16_t rxIterator = 0;
...
void USART3_IRQHandler(void)
{
	...
	if(USART3->SR & (1UL<<5)) // bit  RXNE: Read data register not empty 0: Data is not received  1: Received data is ready to be read.
	{
		...
		if(rxInProgress)
		{
			rxBuffer[rxIterator++] = tempVar;

			if(rxIterator == rxLength)
			{
				rxInProgress = 0;
				xSemaphoreGiveFromISR(xDone, &xHighestPriorityTaskWoken);
			}
		}
	}
